...
def prediction_results():
    # this is your index page
    util.save_model(filename = 'data/hw_regression_data.csv', model_name = 'model/regr.sav')
    # this time our table data has three cols: hour, prediction, truth
    table_data = util.load_model_pred(filename = 'data/test.csv', model_name = 'model/regr.sav')
    # using render_template function, Flask will search
    # the file named index.html under templates folder
    print('tmptmptmptmptmp')
    return json.dumps({'google_data':table_data})
